...
class Ur3TrajectoryPublisher(Node):
    ...
    def __init__(self):
        ...
        self.waypoints = [
            ...
            {"angles": [0.0, -1.0, 1.0, 0.0, 0.0, 0.0], "t": 8},
        ]
        
        # 计算轨迹总时间（取最后一个点的时间）
        self.trajectory_duration = self.waypoints[-1]["t"]
        ...
        self.timer = self.create_timer(
            self.trajectory_duration,  # 参数1: 定时周期（秒）。这里设置为 1.0 秒，意味着 1 秒后会触发一次。
        ...
        self.get_logger().info(f"UR3 Trajectory Publisher 节点已启动！轨迹周期: {self.trajectory_duration}秒")
    ...
    def timer_callback(self):
        ...
        # 遍历预设的轨迹点（waypoints），为每个点创建一个 JointTrajectoryPoint 实例并填充数据。
        for point in self.waypoints:
            traj_point = JointTrajectoryPoint()  # 创建一个新的轨迹点实例。

            # 设置这个轨迹点的关节位置（angles）。
            # 这里假设 point["angles"] 是一个包含六个关节角度的列表，单位是弧度。
            traj_point.positions = point["angles"]

            # 设置这个轨迹点的时间，从轨迹开始到达这个点所需的时间。
            # 这里使用 Duration 消息类型来表示时间段，单位是秒和纳秒。
            traj_point.time_from_start = Duration(sec=point["t"], nanosec=0)

            # 将填充好的轨迹点添加到消息的 points 列表中。
        ...
        self.get_logger().info(f"已经发布轨迹点！执行次数: {self.execution_count}")
